...
- Защитить критические инварианты аутентификации (`auth`) и синхронизации (`sync`) интеграционными тестами (ротация токенов, LWW, изоляция батчей).
- Ожидаемый побочный эффект от покрытия этих сценариев: statement coverage достигнет **~30%**, а endpoint coverage — **~50%**.
